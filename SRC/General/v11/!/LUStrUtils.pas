...
function WordCount (const S: string; const WordDelims: TCharSet;
    DelD: Boolean): Integer;
{ WordCount given a set of word delimiters, returns number of words in S. }

function WordPosition (const N: Integer; const S: string;
    const WordDelims: TCharSet; DelD: Boolean): Integer;
...
function ExtractWordPos (N: Integer; const S: string;
    const WordDelims: TCharSet; var Pos: Integer; DelD: Boolean): string;
...
function GenerateObjectIDStr (AObjectID: TDateTime): string;
...
{$IFDEF WIN32}
    ...
    Result := CompareString (GetThreadLocale, SORT_STRINGSORT or
        NORM_IGNORECASE, PChar(S1), Length(S1), PChar(S2), Length(S2)) - 2;
...
begin
    ...
    begin
    { skip over delimiters }
        if (I <= Length(S)) and (S[I] in WordDelims) then
            Inc (I);
        if DelD then
...
begin
    Len := 0;
    I := WordPosition (N, S, WordDelims, DelD);
    if I <> 0 then
    { find the end of the current word }
        while (I <= Length(S)) and not (S[I] in WordDelims) do
        begin
...
begin
    Result := False;
    Count := WordCount (S, WordDelims, DelD);
    for I := 1 to Count do
        if ExtractWord (I, S, WordDelims, DelD) = W then
        begin
            Result := True;
            Exit;
        end;
...
begin
    ...
    if Length (Key) > 0 then
        for I := 1 to Length(Src) do
            Result[I] := Chr (Byte(Key[1 + ((I - 1) mod Length(Key))])
                xor Ord(Src[I]));
      { Result[I] := ' '; }
end;
...
function WideCharToStr (WStr: PWideChar; Len: Integer): string;
begin
    if Len = 0 then
        Len := - 1;
    Len := WideCharToMultiByte (CP_ACP, 0, WStr, Len, nil, 0, nil, nil);
    SetLength (Result, Len - 1);
    WideCharToMultiByte (CP_ACP, 0, WStr, Len, PAnsiChar(Result), Len,
        nil, nil);
end;

function GenerateObjectIDStr (AObjectID: TDateTime): string;
var
    LAObjectID: TDateTime;
    hh, nn, ss, ms: word;
    s: string;
begin
    LAObjectID := AObjectID;
    DecodeTime (LAObjectID, hh, nn, ss, ms);
    s := AddChar ('0', IntToStr(ms), 3) + ' ';
    //LObjectIDStr := FormatDateTime ('dd.mm.yyyy hh:mm:ss.z', LObjectID);
    Result := FormatDateTime ('ddmmyyyyhhmmss'+s, LAObjectID);
end;
