...
function WordCount(const S: string; const WordDelims: TCharSet; DelD: Boolean): Integer;
...
function ExtractWord(N: Integer; const S: string;
  const WordDelims: TCharSet; DelD: Boolean): string;
...
function IsWordPresent(const W, S: string; const WordDelims: TCharSet; DelD: Boolean): Boolean;
...
begin
  ...
  while I <= SLen do begin
    if DelD then
       while (I <= SLen) and (S[I] in WordDelims) do Inc(I);
//    while (I <= SLen) and (S[I] in WordDelims) do Inc(I);
    if I <= SLen then Inc(Result); inc(i);
    while (I <= SLen) and not(S[I] in WordDelims) do Inc(I);
  end;
end;
(*
...
begin
  ...
  while (I <= Length(S)) and (Count <> N) do begin
    { skip over delimiters }
    if (I <= Length(S)) and (S[I] in WordDelims) then Inc(I);
    if DelD then
       while (I <= Length(S)) and (S[I] in WordDelims) do Inc(I);
//    while (I <= Length(S)) and (S[I] in WordDelims) do Inc(I);
    { if we're not beyond end of S, we're at the start of a word }
...
begin
  ...
  for I := 1 to Count do
    if ExtractWord(I, S, WordDelims, DelD) = W then begin
      Result := True;
      Exit;
    end;
end;
    ...
    begin
      if (AnsiCompareText(Copy(S, 2, MaxInt), Switch) = 0) then begin
        ...
        if I <= ParamCount then begin
          Result := ParamStr(I);
          if (Result[1] in SwitchChars) then Result := '';
          Exit;
        end;
      end;
    end;
