...
type
    ...
    private
        FYouTubeObjects: TYouTubeObjects;

    protected
    public
        constructor Create (AOwner: TCollection); override;
        destructor Destroy; override;
        property YouTubeObjects: TYouTubeObjects read FYouTubeObjects;
    end;
...
begin
    inherited Create (AOwner);
    FYouTubeObjects := TYouTubeObjects.Create (nil);
end;
...
begin
    ...
    for i:=0 to Count-1 do
    begin
        LURL := Items[i].YouTubeObjects.FURL;
        if LURL = AURL then
        begin
            Result := Items[i];
            Break;
